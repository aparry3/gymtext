...
Start from `.env.example`, keep secrets only in `.env.local`. Run `pnpm backup:local` before destructive schema work, and prefix sensitive scripts with `source .env.local && …` (for example `source .env.local && pnpm test:user:create`). Rotate OpenAI/Gemini/Pinecone credentials immediately after any suspected leak.
