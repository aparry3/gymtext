...
/**
 * Proxy endpoint to trigger daily message for a specific user
...
export async function POST(
  ...
  try {
    ...
    const cronUrl = `${webAppUrl}/api/cron/daily-messages/${userId}`;

    console.log(`[ADMIN CRON] Triggering daily message for user ${userId}:`, cronUrl);
    ...
    if (!response.ok) {
      console.error('[ADMIN CRON] Daily message request failed:', result);
      return NextResponse.json(
    ...
    console.log(`[ADMIN CRON] Daily message triggered successfully for user ${userId}:`, result);
    return NextResponse.json(result);

  } catch (error) {
    console.error('[ADMIN CRON] Error triggering daily cron for user:', error);
