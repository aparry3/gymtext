...
import { ProgramFilters, SchedulingMode, AdminProgramOwner } from './types'
...
export function ProgramsFilters({ onFiltersChange, isLoading = false, owners = [] }: ProgramsFiltersProps) {
  ...
  const [filters, setFilters] = useState<ProgramFilters>({
    ...
    schedulingMode: (searchParams.get('schedulingMode') as SchedulingMode) || undefined,
  ...
  useEffect(() => {
    ...
    const url = params.toString() ? `?${params.toString()}` : '/programs'
    router.replace(url, { scroll: false })
    onFiltersChange(filters)
  }, [filters, onFiltersChange, router])

  const handleFilterChange = useCallback((key: keyof ProgramFilters, value: boolean | string | undefined) => {
    setFilters(prev => ({ ...prev, [key]: value }))
  }, [])

  const removeFilter = useCallback((key: keyof ProgramFilters) => {
  ...
  const schedulingModes: SchedulingMode[] = ['rolling_start', 'cohort']
    ...
    <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {schedulingModes.map((mode) => (
                    <Button
                      key={mode}
                      variant={filters.schedulingMode === mode ? "default" : "outline"}
                      size="sm"
                      onClick={() => handleFilterChange('schedulingMode',
                        filters.schedulingMode === mode ? undefined : mode
                      )}
                    >
                      {modeLabels[mode]}
                    </Button>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    size="sm"
                    onClick={() => handleFilterChange('isPublic',
      ...
      {activeFiltersCount > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {filters.ownerId && (
            <FilterBadge
              label={`Owner: ${getOwnerName(filters.ownerId)}`}
              onRemove={() => removeFilter('ownerId')}
            />
          )}
          {filters.schedulingMode && (
            <FilterBadge
              label={`Mode: ${modeLabels[filters.schedulingMode]}`}
              onRemove={() => removeFilter('schedulingMode')}
            />
          )}
          ...
          {filters.isPublic !== undefined && (
            <FilterBadge
              label={filters.isPublic ? 'Public' : 'Private'}
              onRemove={() => removeFilter('isPublic')}
            />
          )}
        </div>
      )}
