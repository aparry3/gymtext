...
/**
 * Endpoint for triggering weekly check-in message for a specific user
...
export async function POST(
  ...
  try {
    ...
    console.log(`[CRON] Triggering weekly message for user ${userId}`, { forceImmediate });

    const services = getServices();
    const result = await services.weeklyMessage.triggerForUser(userId, { forceImmediate });

    console.log(`[CRON] Weekly message trigger result for user ${userId}:`, result);

    return NextResponse.json(result);
  } catch (error) {
    console.error('[CRON] Error triggering weekly message for user:', error);
