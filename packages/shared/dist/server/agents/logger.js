 ...
 */
export function logAgentInvocation(agentName, input, messages, output) {
    ...
    (async () => {
        try {
            ...
            await Promise.all([
                fs.writeFile(path.join(LOGS_DIR, `${baseFilename}.json`), generateJsonLog(agentName, isoTimestamp, input, messages, output), 'utf-8'),
                fs.writeFile(path.join(LOGS_DIR, `${baseFilename}.txt`), generateTextLog(agentName, isoTimestamp, input, messages, output), 'utf-8'),
            ]);
        }
        catch (error) {
