...
export const onboardUserFunction = inngest.createFunction({
    ...
    retries: 2,
}, { event: 'user/onboarding.requested' }, async ({ event, step }) => {
    ...
    try {
        ...
        const { user: initialUser, signupData } = await step.run('step-1-load-data', () => onboardingSteps.loadData(userId));
        // Step 2: Get or create profile (returns updated user)
        // forceCreate=true will always create new profile even if one exists
        const { user } = await step.run('step-2-profile', () => onboardingSteps.getOrCreateProfile(initialUser, signupData, forceCreate));
        // Step 3: Get or create plan (uses user with profile)
        // forceCreate=true will always create new plan even if one exists
        const { plan } = await step.run('step-3-plan', () => onboardingSteps.getOrCreatePlan(user, forceCreate));
        // Step 4: Get or create microcycle (needs plan)
        // forceCreate=true will always create new microcycle even if one exists
        const { microcycle } = await step.run('step-4-microcycle', () => onboardingSteps.getOrCreateMicrocycle(user, plan, forceCreate));
        // Step 5: Get or create workout (needs microcycle)
        // forceCreate=true will always create new workout even if one exists
        await step.run('step-5-workout', () => onboardingSteps.getOrCreateWorkout(user, microcycle, forceCreate));
        // Step 6: Mark completed
        await step.run('step-6-complete', () => onboardingSteps.markCompleted(userId));
        // Step 7: Send messages
        const messagesSent = await step.run('step-7-messages', () => onboardingSteps.sendMessages(userId));
        ...
        try {
            await onboardingDataService.updateStatus(userId, 'failed', error instanceof Error ? error.message : 'Unknown error during onboarding');
        }
        catch (updateError) {
