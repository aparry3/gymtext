...
export class UserRepository extends BaseRepository {
    async list(params) {
        ...
        if (params.hasProfile === true) {
            query = query.where((eb) => eb.exists(eb.selectFrom('profiles')
                .whereRef('profiles.clientId', '=', 'users.id')
                .select('profiles.id')));
        }
        else if (params.hasProfile === false) {
            query = query.where((eb) => eb.not(eb.exists(eb.selectFrom('profiles')
                .whereRef('profiles.clientId', '=', 'users.id')
                .select('profiles.id'))));
        }
        ...
        if (createdFrom)
            countBuilder = countBuilder.where('users.createdAt', '>=', new Date(createdFrom));
        if (createdTo)
            countBuilder = countBuilder.where('users.createdAt', '<=', new Date(createdTo));
        // hasProfile filter checks for existence in profiles table
        if (params.hasProfile === true) {
            countBuilder = countBuilder.where((eb) => eb.exists(eb.selectFrom('profiles')
                .whereRef('profiles.clientId', '=', 'users.id')
                .select('profiles.id')));
        }
        else if (params.hasProfile === false) {
            countBuilder = countBuilder.where((eb) => eb.not(eb.exists(eb.selectFrom('profiles')
                .whereRef('profiles.clientId', '=', 'users.id')
                .select('profiles.id'))));
        }
        ...
        const usersRows = await query
            ...
            .limit(pageSize)
            .execute();
        const users = usersRows.map((u) => (UserModel.fromDb(u))).filter((u) => u !== undefined);
        return { users, total };
    }
    ...
    async findUsersByTimezones(timezones) {
        ...
        const users = await this.db
            ...
            .selectAll('users')
            .execute();
        return users.map(u => UserModel.fromDb(u)).filter(u => u !== undefined);
    }
