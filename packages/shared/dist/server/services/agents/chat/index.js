...
export class ChatService {
    ...
    static async handleIncomingMessage(user) {
        try {
            ...
            const agentContext = await ContextService.getInstance().getContext(userWithProfile, [ContextType.DATE_CONTEXT, ContextType.CURRENT_WORKOUT]);
            // Convert previous messages to Message format for the configurable agent
            const previousMsgs = ConversationFlowBuilder.toMessageArray(context || [])
                .map(m => ({
                role: m.role,
                content: m.content,
            }));
