 ...
 */
export function toolWithMessage(name, description, callback, onSendMessage) {
    return tool(async (args) => {
        ...
        description,
        schema: z.object({
            message: z.string().describe('REQUIRED. Brief acknowledgment to send immediately (1 sentence). Example: "Got it, switching to legs!"'),
        }),
    });
}
/**
 ...
 */
export const createChatTools = (context, deps, onSendMessage) => {
    // Tool 1: Update Profile (Priority 1 - runs first when called with other tools)
    const updateProfileTool = tool(async () => {
...
All context (user, message, date, etc.) is automatically provided - no parameters needed.`, async () => {
    ...
    // Tool 3: Get Workout
    const getWorkoutTool = tool(async () => {
