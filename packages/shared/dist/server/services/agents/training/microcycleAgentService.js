...
import { MicrocycleStructureSchema } from '@/server/models/microcycle';
import { MicrocycleGenerationOutputSchema, microcycleMessageUserPrompt, structuredMicrocycleUserPrompt, ModifyMicrocycleOutputSchema, } from '@/server/services/agents/prompts/microcycles';
...
export class MicrocycleAgentService {
    ...
    async getStructuredAgent() {
        if (!this.structuredAgentPromise) {
            this.structuredAgentPromise = createAgent({
                ...
                userPrompt: (input) => {
                    const data = JSON.parse(input);
                    return structuredMicrocycleUserPrompt(data.overview, data.days, data.absoluteWeek ?? 1, data.isDeload);
                },
    ...
    async generateMicrocycle(user, absoluteWeek) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                if (!validateDays(result.response.days)) {
                    ...
                    throw new Error(`Microcycle generate validation failed: Missing or empty days for ${missingDays.join(', ')}. ` +
        ...
        throw new Error(`Failed to generate microcycle pattern for week ${absoluteWeek} after ${MAX_RETRIES} attempts. ` +
    ...
    async modifyMicrocycle(user, currentMicrocycle, changeRequest) {
            ...
            try {
                ...
                if (!validateDays(result.response.days)) {
                    const emptyDayIndices = result.response.days
                        ...
                        .filter(index => index !== -1);
                    const missingDays = emptyDayIndices.map(index => DAY_NAMES[index]);
                    throw new Error(`Microcycle modify validation failed: Missing or empty days for ${missingDays.join(', ')}. ` +
                        `Expected all 7 days to be present and non-empty.`);
                }
        ...
        // All retries exhausted
        throw new Error(`Failed to modify microcycle pattern for week ${absoluteWeek} after ${MAX_RETRIES} attempts. ` +
            `Last error: ${lastError?.message || 'Unknown error'}`);
    }
}
