...
export class MessageService {
     ...
     */
    async storeOutboundMessage(clientId, to, messageContent, from = getTwilioSecrets().phoneNumber, provider = 'twilio', providerMessageId, metadata) {
    ...
    async sendMessage(user, message, mediaUrls) {
        ...
        let stored = null;
        try {
            stored = await this.storeOutboundMessage(user.id, user.phoneNumber, message || '[MMS only]', // Store placeholder for MMS-only messages
