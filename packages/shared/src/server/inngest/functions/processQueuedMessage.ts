 ...
 * 3. Send message via MessagingOrchestrator
...
export const processNextQueuedMessageFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run('process-next-message', async () => {
      ...
      await services.messagingOrchestrator.processNext(clientId, queueName);
    });
...
export const sendQueuedMessageFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run('send-message', async () => {
      ...
      return await services.messagingOrchestrator.sendQueuedMessage(queueEntryId);
    });
