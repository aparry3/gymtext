#!/usr/bin/env tsx
/**
 * Anonymized Production Data Pipeline
 *
 * Dumps production data, restores to target database, prunes to top N users,
 * and anonymizes all PII. Works for both local dev and remote staging — just
 * source the appropriate env file before running.
 *
 * Usage:
 *   source .env.local   && pnpm db:anonymize          # local dev
 *   source .env.staging && pnpm db:anonymize          # remote staging
 ...
 *   DATABASE_URL         — target database to populate
 */
...
if (!PROD_URL) {
  ...
  console.error('Set a read-only connection string to production.');
  exit(1);
}
...
async function recreateDatabase() {
  banner(`Drop and recreate database: ${DB_NAME}`);

  if (DRY_RUN) {
    console.log('[dry-run] Would drop and recreate database');
    return;
  }
...
async function dumpAndRestore() {
  ...
  if (DRY_RUN) {
    console.log('[dry-run] Would dump production and restore to target');
    return;
  }
...
async function selectAndPruneUsers(db: Kysely<any>, count: number) {
  banner(`Select top ${count} users and prune the rest`);

  if (DRY_RUN) {
    const total = await sql<{ count: number }>`SELECT count(*)::int as count FROM users`.execute(db);
    console.log(`[dry-run] Would keep ${count} of ${total.rows[0]?.count ?? '?'} users`);
    return;
  }
...
async function anonymizeDatabase(db: Kysely<any>) {
  banner('Anonymize database');

  if (DRY_RUN) {
    console.log('[dry-run] Would anonymize all PII in target database');
    return;
  }
...
// ---------------------------------------------------------------------------
// Main (exported for use by reset-from-main.ts)
// ---------------------------------------------------------------------------
...
export async function runAnonymize() {
  ...
  console.log(`\nAnonymize pipeline complete in ${elapsed}m`);
...
const isDirectRun = process.argv[1]?.endsWith('anonymize.ts');
if (isDirectRun) {
  runAnonymize().catch(error => {
