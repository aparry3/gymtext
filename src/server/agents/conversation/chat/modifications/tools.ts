...
const SubstituteExerciseSchema = z.object({
  ...
  replacementExercise: z.string().optional().describe('Optional: specific exercise to replace it with. If not provided, the AI will suggest alternatives.'),
  reason: z.string().describe('Reason for the substitution (e.g., "No barbell available", "Shoulder injury")'),
...
/**
 * Tool for substituting a specific exercise or block in the current workout
 * Use this when the user wants to swap out one exercise for another
 * Example: "Can I swap barbell bench press for dumbbell bench press?"
 */
export const substituteExerciseTool = new DynamicStructuredTool({
  ...
  func: async ({ userId, workoutDate, exerciseToReplace, replacementExercise, reason, blockName }: SubstituteExerciseInput) => {
    ...
    if (result.success) {
      return `Successfully replaced "${exerciseToReplace}" with "${result.replacementExercise}". ${result.message || ''}`;
    } else {
 ...
 */
export const modifyWorkoutTool = new DynamicStructuredTool({
  name: 'modify_workout',
 ...
 */
export const modifyWeekTool = new DynamicStructuredTool({
  name: 'modify_week',
