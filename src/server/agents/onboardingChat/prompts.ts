...
//     let goalText = `Goal: ${profile.goals.primary}`;
//     if (profile.goals.specific) {
//       goalText += ` (${profile.goals.specific})`;
//     }
//     if (profile.goals.timeline) {
//       goalText += ` - ${profile.goals.timeline} week timeline`;
//     }
//     keyFacts.push(goalText);
//   }
  
//   // Current activity data
//   if (profile.activityData && profile.activityData.length > 0) {
//     const activities = profile.activityData.map(a => `${a.type} (${a.experience} level)`).join(', ');
//     keyFacts.push(`Activities: ${activities}`);
//   }
...
//       keyFacts.push(`Constraints: ${activeConstraints.map(c => c.description).join(', ')}`);
//     }
//   }
  
//   // Equipment access
//   if (profile.equipmentAccess) {
//     const access = profile.equipmentAccess.gymAccess ? 'Gym access' : 'No gym access';
//     keyFacts.push(`Equipment: ${access}`);
//   }
  
//   // Availability 
//   if (profile.availability) {
//     keyFacts.push(`Availability: ${profile.availability.daysPerWeek} days/week, ${profile.availability.minutesPerSession} min/session`);
//   }
