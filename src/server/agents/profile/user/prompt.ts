...
/**
 * Build the system prompt for the UserAgent using full user context
 * This agent specializes in extracting user demographics and contact information
 * Uses both update_user_profile and update_user_info tools
 */
export const buildUserPromptWithContext = (user: UserWithProfile): string => {
  const userJson = {
    name: user.name,
    age: user.age,
    gender: user.gender,
...
You are a USER INFO extraction specialist for GymText, a fitness coaching app.
