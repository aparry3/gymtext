...
export const createStructuredMicrocycleAgent = <TPattern = MicrocyclePattern>(
  ...
  return createRunnableAgent<MicrocycleChainContext, TPattern>(async (input) => {
    ...
    console.log(`[${config.operationName}] Generated structured microcycle pattern for week ${weekNumber}`);

    // Add weekIndex deterministically (not generated by AI) - weekNumber is already 1-based
    return {
      ...validatedPattern,
